...
const MoiDelish = () => {
    ...
    useEffect(() => {
        const fetchVendors = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/food/vendors/public/approved');
                if (!response.ok) {
                ...
                {filteredVendors.length > 0 ? (
                    filteredVendors.map((vendor, index) => (
                      ...
                      <div className="card-body">
                        {/* Vendor Description - Independent and positioned above */}
                        <p className="vendor-description mb-2">
                          {vendor.description || "Delicious food available"}
                        </p>
                    
                        {/* Vendor Location and "Open" Status */}
                        {vendor.location && (
                          <div className="flex justify-between items-center">
                            ...
                            <div className="flex items-center text-green-600">
                              <i className="fas fa-check-circle mr-2"></i> {/* Icon for "Open" */}
                              <span>Open</span>
                            </div>
                          </div>
                        )}
  ...
  View Menu
                        ...
                        <button 
                          className="cta-button"
                          onClick={() => window.location.href = `tel:${vendor.phone || vendor.contactNumber || ""}`}
                        >
